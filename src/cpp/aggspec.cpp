...
t_aggspec::t_aggspec(
    const t_str& name, t_aggtype agg, const t_depvec& dependencies)
    : m_name(name)
    , m_disp_name(name)
    , m_agg(agg)
...
t_aggspec::t_aggspec(const t_str& aggname, const t_str& disp_aggname,
    t_aggtype agg, t_uindex agg_one_idx, t_uindex agg_two_idx,
    t_float64 agg_one_weight, t_float64 agg_two_weight)
    : m_name(aggname)
