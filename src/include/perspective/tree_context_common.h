...
PERSPECTIVE_EXPORT void notify_sparse_tree(t_stree_sptr tree,
    ...
    const t_sortsvec& ctx_sortby, const t_table& flattened, const t_table& prev,
    const t_table& current, const t_config& config, const t_gstate& gstate);
...
void
ctx_expand_path(CONTEXT_T& ctx, t_header header, t_stree_sptr tree,
    t_trav_sptr traversal, const t_tscalvec& path)
{
    t_index root = 0;
    t_tvidx bidx = 0;
