...
template <typename CTX_T, typename PKEY_TYPE_T = t_int64, int PKEY_DTYPE = DTYPE_INT64>
...
do_pivot(t_do_pivot mode, const t_table& tbl, const t_config& cfg) {
    ...
    auto pkey_col = copy->add_column("psp_pkey", static_cast<t_dtype>(PKEY_DTYPE), true);
